...
// Expects the element using this `phx-hook=` to pass 
// a character limit in `data-value=`.
Hooks.CharacterCounter = {
  mounted() {
    this.el.addEventListener("input", e => {
      ...
      if (trimmedLength == 0) {
        label.textContent = label.textContent.replace(/\s\([\s\S]+\)\s*$/, "")
        return;
      }

      if (label.textContent.match(/characters?\)\s*$/)) {
        ...
        label.textContent += " (" + trimmedLength + "/" + 
                             this.el.dataset.value + " character" + s + ")"
      } 
    })
  }
}
