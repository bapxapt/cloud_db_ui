defmodule CloudDbUi.AccountsTest do
  ...
  describe "list_users_for_index/0" do
    test "returns all users with preloads" do
      user = user_fixture()

      user_with_order =
        Map.replace!(user, :orders, [order_fixture(%{user_id: user.id})])

      assert Accounts.list_users_for_index() == [user_with_order]
    end
  end

  test "get_user_for_show!/1 returns a user with preloads" do
    user = user_fixture()

    user_with_order =
      Map.replace!(user, :orders, [order_fixture(%{user_id: user.id})])

    assert Accounts.get_user_for_show!(user.id) == user_with_order
  end
