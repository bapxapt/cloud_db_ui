defmodule CloudDbUi.AccountsTest do
  ...
  describe "list_users_with_order_count/0" do
    test "returns all users with order count" do
      ...
      order_paid_fixture(%{user_id: user.id})
      ...
      assert Accounts.list_users_with_order_count() == [user_new]
    end
  end
  ...
  test "get_user_with_order_suborder_products!/1 returns a user w/ preloads" do
    ...
    order_new =
      Map.replace!(order, :suborders, [Map.replace!(suborder, :product, prod)])

    user_new =
      user
      |> Map.replace!(:orders, [order_new])
      |> Map.replace!(:paid_orders, 0)

    assert Accounts.get_user_with_order_suborder_products!(user.id) == user_new
  end
  ...
  describe "register_user/1" do
    ...
    test "validates email and password when given" do
      {:error, changeset} =
        Accounts.register_user(%{email: "not valid", password: "Short1"})
  ...
  describe "update_user_password/3" do
    ...
    test "validates maximum values for password for security", %{user: user} do
      ...
      assert "should be at most 72 character(s)" in password_errors
    end
