defmodule CloudDbUi.DataCase do
  ...
  """
  @spec set_as_paid(%Order{}, %User{}, %DateTime{}) :: %Order{}
  def set_as_paid(
        %Order{paid_at: nil} = order,
        %User{} = user,
        at \\ ~U[2020-01-01 00:00:00Z]
      ) when order.user_id == user.id do
    {:ok, paid} = Orders.update_order(order, %{paid: true, paid_at: at}, user)

    paid
  end

  @doc """
  Set an existing product type as non-assignable. No other changes.
  """
  @spec set_as_unassignable(%ProductType{}) :: %ProductType{}
  def set_as_unassignable(%ProductType{assignable: true} = type) do
    {:ok, type_new} = Products.update_product_type(type, %{assignable: false})
