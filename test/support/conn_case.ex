defmodule CloudDbUiWeb.ConnCase do
  ...
  def has_table_cell?(%View{} = live, selector \\ nil, filter) do
    has_element?(live, "#{if selector, do: selector <> " "} td", filter)
  end
  ...
  For example, in `"#users-100"` such a part is `"#users-"`.
  """
  ...
  def has_table_row?(%View{} = live, selector_part, object, fields) do
    has_table_row?(
      live,
  ...
  def assert_filter_form_errors(%View{} = _live, _index, _type), do: true
  ...
  """
  @spec assert_sort_param_handling(%Plug.Conn{}, String.t()) :: boolean()
  def assert_sort_param_handling(%Plug.Conn{} = conn, path) do
    [
      %{"order" => "non_existing"},
      %{"order" => ["non_existing"]},
      %{"order" => [["non_existing"]]},
      %{"order_by" => "non_existing"},
      %{"order_by" => ["non_existing"]},
      %{"order_by" => [["non_existing"]]},
      %{"order_by" => "balance"},
      %{"order_by" => ["balance"]},
      %{"order_by" => [["balance"]]},
      %{"order_by" => ["non_existing"], "order_directions" => ["asc"]},
      %{"order_by" => ["balance", "id"], "order_directions" => ["asc"]},
      %{"order_by" => ["balance"], "order_directions" => ["asc", "desc"]},
      %{"order_by" => ["balance"], "order_directions" => ["BAD"]},
      %{"order_by" => ["balance", "id"], "order_directions" => "NONE"},
      %{"order_by" => ["BAD", "id"], "order_directions" => ["asc", "NONE"]},
      %{"order_by" => ["BAD", "id"], "order_directions" => "NOT_A_LIST"},
      %{"order_directions" => "desc"},
      %{"order_directions" => ["desc"]},
      %{"order_directions" => [["desc"]]}
    ]
    |> Enum.all?(&assert_page_has_pagination_counter(conn, path, &1))
  end
  ...
  """
  @spec assert_table_row_count(%View{}, non_neg_integer()) :: boolean()
  def assert_table_row_count(%View{} = live, count) do
    assert_table_row_count(live, "tbody", count)
  end

  # A table is supposed to be not displayed if there are no results.
  @spec assert_table_row_count(%View{}, String.t(), non_neg_integer()) ::
          boolean()
  def assert_table_row_count(%View{} = live_view, tbody_selector, count) do
    assert(has_n_children?(live_view, tbody_selector, count))
  ...
  defp assert_filter_field_param_handling(conn, path, index, field) do
    index
    |> malformed_field_url_params(field)
    |> Kernel.++(malformed_field_url_params(index, "#{field}!!"))
    |> Enum.all?(&assert_page_has_pagination_counter(conn, path, &1))
  end
  ...
  defp assert_filter_value_param_handling(conn, path, index, field, op) do
    assert_page_has_pagination_counter(
      conn,
      path,
      filter_params(index, field, op, "__UNEXPECTED__")
    )
  end

  # This means the page loaded successfully.
  # A URL `path` should not begin with a forward slash.
  @spec assert_page_has_pagination_counter(%Plug.Conn{}, String.t(), map()) ::
          boolean()
  defp assert_page_has_pagination_counter(%Plug.Conn{} = conn, path, params) do
    {:ok, lv, _html} = live(conn, ~p"/#{path}?#{params}")

    assert(has_element?(lv, "#pagination-counter", "result"))
  end
  ...
  defp create_paid_order_with_suborder(_context, %User{} = user) do
    ...
    %{order: CloudDbUi.DataCase.set_as_paid(order, user), suborder: suborder}
  end
  ...
  @spec malformed_field_url_params(non_neg_integer(), atom()) :: [any()]
  defp malformed_field_url_params(index, field) when is_atom(field) do
    malformed_field_url_params(index, "#{field}")
  end

  @spec malformed_field_url_params(non_neg_integer(), String.t()) :: [any()]
  defp malformed_field_url_params(index, field) when is_binary(field) do
    [
      "",
      field,
      [field],
