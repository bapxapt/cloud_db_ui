defmodule CloudDbUiWeb.UserSettingsLiveTest do
  ...
  describe "confirm email" do
    ...
    test "updates a user's e-mail address once",
      ...
      assert(flash["info"] == "E-mail changed successfully.")
      ...
      assert(path == ~p"/users/settings")
      assert(flash["error"] =~ "E-mail change link is invalid or it has expir")
    end

    test "does not update the e-mail address with an invalid token",
         %{conn: conn, user: user} do
      {:error, {:live_redirect, %{to: path, flash: flash}}} =
        live(conn, ~p"/users/settings/confirm_email/BAD_TOKEN")
