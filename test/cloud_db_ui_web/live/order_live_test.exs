defmodule CloudDbUiWeb.OrderLiveTest do
  ...
  describe "Index (paid order), a user" do
    setup [:register_and_log_in_user, :create_paid_order_with_suborder]
  ...
  describe "Show (unpaid order), a user" do
    ...
    test "gets redirected away from editing", %{conn: conn, order: order} do
  ...
  describe "Show (unpaid order), an admin" do
    ...
    setup [:create_order, :create_suborder, :register_and_log_in_admin]
  ...
  describe "Show (paid order), an admin" do
    ...
    test "updates a paid order within modal", %{conn: conn, order: order} do
      {:ok, show_live, _html} = live(conn, ~p"/orders/#{order}")
  ...
  defp assert_order_form_errors(%View{} = live) do
    ...
    change_form(live, %{paid_at: ~U[2999-01-01 00:00:00.000+00]})

    live
    |> has_form_error?("#order-form", :paid_at, "can&#39;t be in the future")
    |> assert()
