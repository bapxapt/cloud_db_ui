defmodule CloudDbUi.Changeset do
  ...
  def maybe_validate_format_not_negative_zero(set, attrs, fld, digits \\ 2) do
    cond do
      Keyword.has_key?(set.errors, fld) -> set
      !has_attr?(attrs, fld) -> set
      true -> validate_format_not_negative_zero(set, attrs, fld, digits)
    end
  end
  ...
  def validator_not_in_the_future(field, %DateTime{} = value) do
    ...
    |> case do
      :lt ->
        [{field, {"can't be in the future", [validation: :not_future]}}]
  ...
  defp validate_format_not_negative_zero(changeset, attrs, field, digits) do
    attrs
    |> get_attr(field)
    |> negative_zero?(digits)
    |> case do
      true -> add_error(changeset, field, negative_zero_error())
      false -> changeset
    end
  end
