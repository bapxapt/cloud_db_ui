defmodule CloudDbUi.Accounts do
  ...
  """
  @spec list_users_with_order_count(attrs() | %Flop{}) ::
          {:ok, {[%User{}], %{atom() => any()}}} | {:error, %{atom() => any()}}
  def list_users_with_order_count(params) do
  ...
  @doc """
  Create a user (that can possibly be an administrator), for exmaple,
  ...
  """
  @spec change_user(%User{}, attrs(), boolean()) :: %Changeset{}
  def change_user(%User{} = user, attrs \\ %{}, validate_unique? \\ true) do
    User.validation_changeset(user, attrs, validate_unique?)
  end
