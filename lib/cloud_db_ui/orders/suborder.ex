defmodule CloudDbUi.Orders.SubOrder do
  ...
  defp validate_number_quantity(changeset) do
    ...
    |> validate_number(
      ...
      message: "cannot order more than 100 000 pieces"
    )
  end
  ...
  defp maybe_validate_order_id(%{changes: %{order_id: _id}} = set, order) do
    cond do
      !order ->
        add_error(set, :order_id, "order not found")
  ...
  defp maybe_validate_product_id(%{changes: %{product_id: _}} = set, prod) do
    cond do
      !prod ->
        add_error(set, :product_id, "product not found")

      !prod.orderable ->
        add_error(set, :product_id, "cannot assign a non-orderable product")
