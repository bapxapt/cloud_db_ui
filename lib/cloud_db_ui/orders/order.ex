defmodule CloudDbUi.Orders.Order do
  ...
  def saving_changeset(order, attrs, user) do
    ...
    # TODO: remove
    |> tap(fn a ->
      IO.puts("\n\n    IN saving_changeset() BEFORE maybe_replace_and_validate_user_id()")
      IO.inspect(a)
      IO.puts("\n\n")
    end)
    ...
    # TODO: remove
    |> tap(fn a ->
      IO.puts("\n\n    IN saving_changeset() AFTER maybe_replace_and_validate_user_id()")
      IO.inspect(a)
      IO.puts("\n\n")
    end)
  ...
  defp maybe_validate_user_id(%{changes: %{user_id: user_id}} = set, user) do
    trimmed = String.trim(user_id)

    cond do
      trimmed == "" ->
        add_error(set, :user_id, "can't be blank")

      String.length(trimmed) > 160 ->
        add_error(set, :user_id, "should be at most 160 characters")

      !valid_id?(trimmed) and !User.valid_email?(trimmed) ->
        add_error(set, :user_id, "neither an ID nor a valid e-mail")

      !user ->
        add_error(set, :user_id, "user not found")

      user.admin ->
        add_error(set, :user_id, "cannot assign an order to an administrator")
