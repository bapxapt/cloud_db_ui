defmodule CloudDbUi.Accounts.User do
  ...
  @derive {
    ...
    adapter_opts: [custom_fields: custom_field_opts()],
    default_limit: 25,
    max_limit: 100,
  ...
  def validation_changeset(%__MODULE__{} = user, attrs, validate_unique?) do
    ...
    |> maybe_validate_zero_admin_balance(user, attrs)
  ...
  @spec maybe_validate_zero_admin_balance(
          %Changeset{},
          %__MODULE__{},
          attrs()
        ) :: %Changeset{}
  defp maybe_validate_zero_admin_balance(
    ...
    case get_attr(attrs, :admin) || user.admin do
      truthy when truthy in [true, "true"] -> validate_zero_admin_balance(set)
      _any -> set
    end
  end

  # No change of `:balance` in change`set.changes`.
  defp maybe_validate_zero_admin_balance(set, %__MODULE__{}, _attrs), do: set

  # TODO: try to pass balance as the second parameter

  @spec validate_zero_admin_balance(%Changeset{}) :: %Changeset{}
  defp validate_zero_admin_balance(%{changes: %{balance: balance}} = set)
       when is_binary(balance) do
    with trimmed <- String.trim(balance),
         {%Decimal{} = parsed, ""} <- parse_decimal(trimmed),
         :gt <- Decimal.compare(parsed, 0) do
      add_error(
        set,
        :balance,
        "can't be non-zero for an admin",
        [validation: :not_admin]
      )
    else
      _ -> set
    end
  ...
  defp validate_zero_admin_balance(%{changes: %{balance: %Decimal{}}} = set) do
    if Decimal.compare(set.changes.balance, 0) == :eq do
      set
    else
      add_error(
        set,
