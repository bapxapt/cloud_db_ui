defmodule CloudDbUi.Products.Product.Query do
  ...
  @spec fragment_paid_orders() :: Macro.t()
  defmacrop fragment_paid_orders() do
    quote do
      filter(count(o, :distinct), not is_nil(o.paid_at))
    end
  end
  ...
  def with_order_count(query \\ base()) do
    ...
    # TODO: two select_merge()s?

    |> select_merge(
      [product: p, order: o],
      %{orders: count(o, :distinct), paid_orders: fragment_paid_orders()}
    )
  end
