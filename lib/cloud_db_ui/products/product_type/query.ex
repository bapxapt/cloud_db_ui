defmodule CloudDbUi.Products.ProductType.Query do
  ...
  @doc """
  Only `{"id", "name"}` of assignable product types.
  """
  @spec assignable_ids_names(%Ecto.Query{}) :: %Ecto.Query{}
  def assignable_ids_names(query \\ base()) do
    query
    |> select([type: t], {type(t.id, :string), t.name})
    |> where([type: t], t.assignable == true)
  end
