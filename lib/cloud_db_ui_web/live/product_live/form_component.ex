defmodule CloudDbUiWeb.ProductLive.FormComponent do
  ...
  # TODO: open a new product form when there are no assignable types, press "Save" to make the error flash appear,
    # TODO: quickly close the new product form and open it again (crash),
    # TODO: or do not open (wait for "send_update failed" in the terminal)
      # TODO: maybe some function in CoreComponents that would clear :flash_clear_refs on modal closure?
    # TODO: the crash has been handled with `def clear(%Socket{} = socket, _kind), do: socket` in FlashTimed
      ...
      >
        ...
        <%= if @types != %{} do %>
          <.input
            ...
            type="select"
            label="Type"
            options={Enum.map(@types, fn {id, name} -> {name, id} end)}
            prompt={if @action == :new, do: "—"}
          />
        <% else %>
          <.label :if={@action == :edit}>
            <%= label_no_assignable_product_types(:edit) %>.
          </.label>
          <.error :if={@action == :new}>
            <%= label_no_assignable_product_types(:new) %>
          </.error>
        <% end %>
        <.input
  ...
  def handle_event("save", %{"product" => product_params}, socket) do
     {:noreply, save_product(socket, product_params)}
  end
  ...
  defp validate_product(socket, %{"product" => product_params} = params) do
    ...
    |> maybe_assign_remove_image_state(params["remove-image"])
    |> maybe_cancel_all_uploads(:image, params["remove-image"])
    |> maybe_cancel_invalid_uploads(:image)
    |> assign(:form, to_form(changeset, [action: :validate]))
  end

  # Get a type name if `product_params` contain `"product_type_id"`.
  # Failing that, get `socket.assigns.product.product_type.name`.
  @spec save_product(%Socket{}, params()) :: %Socket{}
  defp save_product(socket, product_params) do
    socket
    |> assign(:type_name, get_type_name(socket, product_params))
    |> save_product(socket.assigns.action, product_params)
  end
  ...
  defp save_product(socket, :new, product_params) do
    ...
    |> Products.create_product(socket.assigns.upload_errors)
    |> handle_saving_result(socket, "Product created successfully.")
  end

  # Success.
  @spec handle_saving_result({:ok, %Product{}}, %Socket{}, String.t()) ::
          %Socket{}
  defp handle_saving_result({:ok, product}, socket, flash_msg) do
    product_new =
      product
      |> Map.replace!(:orders, socket.assigns.product.orders)
      |> Map.replace!(:product_type, %{name: socket.assigns.type_name})

    notify_parent({:saved, product_new})
  ...
  @spec handle_saving_result({:error, %Changeset{}}, %Socket{}, String.t()) ::
          %Socket{}
  defp handle_saving_result({:error, %Changeset{} = set}, socket, _fm) do
    socket
  ...
  # in `product_params`, get it from there.
  @spec get_type_name(%Socket{}, params()) :: String.t() | nil
  defp get_type_name(socket, %{"product_type_id" => id} = _product_params) do
    Map.get(socket.assigns.types, id)
  end

  # The type selector is hidden (no assignable types),
  # no `"product_type_id"` in `product_params`, get it
  # from the `socket` as a fall-back mechanism.
  defp get_type_name(socket, _), do: socket.assigns.product.product_type.name
  ...
  defp maybe_add_image_path(product_params, socket) do
    changeset =
      Products.change_product(socket.assigns.product, product_params)
  ...
  @spec upload_static_file!(
          %{atom() => String.t()},
          %Phoenix.LiveView.UploadEntry{}
        ) :: {:ok, String.t()}
  defp upload_static_file!(%{path: path} = _metadata, _entry) do
    case ImageServer.upload(path) do
      {200, _cont_type, %{"ok" => true, "path" => img_path}} -> {:ok, img_path}
      {:error, reason} -> raise(%HTTPoison.Error{reason: reason})
      _any -> raise(%HTTPoison.Error{reason: "Upload failed."})
    end
  end
  ...
  defp assign_product_types(socket) do
    assign(
      socket,
      :types,
      Map.new(Products.list_assignable_product_type_ids_names())
    )
  end
  ...
  # The "Remove the current image" check box has been rendered.
  # `check_box_state` (`params["remove-image"]`) is a string,
  ...
  # Remove `keys` from `socketassigns.form.errors`.
  @spec delete_form_errors(%Phoenix.HTML.Form{}, [atom()]) ::
          %Phoenix.HTML.Form{}
  defp delete_form_errors(%Phoenix.HTML.Form{errors: errors} = form, keys) do
    Map.replace!(form, :errors, Keyword.drop(errors, keys))
  end

  @spec label_no_assignable_product_types(atom()) ::
          [String.t() | {:safe, list()}]
  defp label_no_assignable_product_types(:new = _action) do
    ["unable to set product type: no ", link_to_product_types()]
  end

  defp label_no_assignable_product_types(:edit = _action) do
