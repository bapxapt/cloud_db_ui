defmodule CloudDbUiWeb.OrderLive.Show do
  ...
  def handle_info(
        {_module, {:saved, %SubOrder{} = suborder_new, quantity_delta}},
        %{assigns: %{suborder: suborder}} = socket
      ) when suborder_new.order_id == suborder.order_id do
    socket_new =
      socket
      |> stream_insert(:suborders, suborder_new)
      |> update_order_total(suborder_new, quantity_delta)
  ...
  # to an other order.
  def handle_info(
        {_module, {:saved, %SubOrder{} = suborder_new, _quantity_delta}},
        socket
      ) do
    socket_new =
      socket
      |> stream_delete(:suborders, suborder_new)
      |> delete_suborder_id(suborder_new)
      |> update_order_total(suborder_new, -suborder_new.quantity)
  ...
  @spec update_order_total(%Socket{}, %SubOrder{}, integer()) :: %Socket{}
  defp update_order_total(%Socket{} = socket, suborder, quantity_delta) do
    update(socket, :order, &update_total(&1, suborder, quantity_delta))
  end

  @spec update_total(%Order{}, %SubOrder{}, integer()) :: %Order{}
  defp update_total(%Order{} = order, suborder, quantity_delta) do
    total =
      order.total
