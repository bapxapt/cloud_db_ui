defmodule CloudDbUiWeb.OrderLive.Actions do
  ...
  def apply_action(
    ...
    order = Orders.get_order_with_suborder_products!(order_id)
  ...
  defp delete_order(%Socket{} = socket, %Order{paid_at: nil} = order) do
    suborder_count = length(order.suborders)
    {^suborder_count, _} = Orders.delete_suborders(order.suborders)
  ...
  defp page_title(:edit_suborder), do: "Edit position"
