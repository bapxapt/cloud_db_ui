defmodule CloudDbUiWeb.OrderLive.PayComponent do
  ...
  defp pay_for_order(%{assigns: %{error_text: text}} = socket, _params)
       when not is_nil(text) do
    FlashTimed.put(socket, :error, String.capitalize(text) <> ".", __MODULE__)
  end

  # An attempt to pay for an unpaid order when `:error_text` is `nil`.
  defp pay_for_order(
  ...
  # Either of the changesets is invalid.
