defmodule CloudDbUiWeb.ProductTypeLive.FormComponent do
  ...
  import CloudDbUi.Changeset, only: [add_unique_constraint_error: 2]
  import CloudDbUi.Queue

  @type params :: CloudDbUi.Type.params()

  # The maximal length of the `:taken_names` queue.
  @taken_names_limit 20
  ...
  defp validate_product_type(
    ...
    |> maybe_add_taken_name(changeset)
  end
  ...
  defp validate_unique_constraint?(name, %Socket{assigns: assigns} = socket) do
    cond do
      ...
      in_queue?(socket, :taken_names, name) -> false
  ...
  defp maybe_add_unique_constraint_error(
    ...
    cond do
      ...
      !in_queue?(socket, :taken_names, name) -> set
      true -> add_unique_constraint_error(set, :name)
    end
  end

  # No change of `:name` in `changeset.changes`.
  defp maybe_add_unique_constraint_error(changeset, _socket), do: changeset

  # Add a `name` to `socket.assigns.taken_names`, if
  # change`set.errors` contain a "has been already taken" error.
  @spec maybe_add_taken_name(%Socket{}, %Changeset{}) :: %Socket{}
  defp maybe_add_taken_name(socket, %{changes: %{name: name}} = set) do
    error = set.errors[:name]

    cond do
      !error -> socket
      elem(error, 1)[:validation] != :unsafe_unique -> socket
      true -> add_to_queue(socket, :taken_names, name, @taken_names_limit)
    end
  end

  # No change of `:name` in `changeset.changes`.
  defp maybe_add_taken_name(socket, _changeset), do: socket
