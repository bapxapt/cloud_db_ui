defmodule CloudDbUiWeb.SubOrderLive.Actions do
  ...
  def apply_action(socket, :new, _params) do
    ...
    |> assign(
      :suborder,
      %SubOrder{product: nil, order: nil, unit_price: "0.00", quantity: 1}
    )
  end
  ...
  def delete_suborder(socket, %{order: %Order{paid_at: nil}} = suborder) do
    {:ok, _deleted_suborder} = Orders.delete_suborder(suborder)
  ...
  def delete_suborder(socket, %{order: %Order{paid_at: _at}} = _suborder) do
