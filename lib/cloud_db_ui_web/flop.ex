defmodule CloudDbUiWeb.Flop do
  ...
  """
  @spec populate_meta_errors(%Socket{}, keyword(keyword()), [String.t()]) ::
          %Socket{}
  ...
  def maybe_push_order(flop, %{"order" => _} = params, sortable_fields) do
    Flop.push_order(
      flop,
      sanitise_order_params(params, sortable_fields)["order"]
    )
  end

  def maybe_push_order(flop, %{"order_by" => _by} = params, sortable_fields) do
    params_new = sanitise_order_params(params, sortable_fields)
  ...
  def maybe_push_order(flop, _params, _sortable_fields), do: flop

  @doc """
  Options of a `type="text"` filter field without `inputmode="decimal"`.
  """
  @spec text_field_opts(String.t(), atom()) :: keyword()
  def text_field_opts(label, op), do: [op: op, label: label, type: "text"]
  ...
  # Check whether an error with a validation in `validations` exists
  # for a specific filter `index` in a `%Meta{}`.
  #@spec has_meta_error?(%Meta{}, non_neg_integer(), [atom()]) :: boolean()
  #defp has_meta_error?(%Meta{} = meta, filter_index, validations) do
  #  meta
  #  |> get_filter_value_errors(filter_index)
  #  |> case do
  #    [] ->
  ...
  #
  #    errors ->
  #      Enum.any?(errors, fn {_, info} -> info[:validation] in validations end)
  #  end
  #end
  ...
  # `form_field_opts` are what is passed into `Flop.Phoenix.filter_fields`
  # as `fields={}`.
  @spec min_max_field_indices([String.t()], keyword(keyword())) ::
          [{non_neg_integer(), non_neg_integer()}]
  defp min_max_field_indices(text_labels, form_field_opts) do
    text_labels
    |> Enum.map(&find_field_index(form_field_opts, &1))
    |> Enum.chunk_every(2)
  ...
  @spec find_field_index(keyword(keyword()), String.t()) ::
          non_neg_integer() | nil
  defp find_field_index(form_field_opts, label_text) do
    form_field_opts
    |> field_indices(&(&1[:label] == label_text))
    |> List.first()
  end
