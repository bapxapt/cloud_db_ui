defmodule CloudDbUiWeb.CoreComponents do
  ...
  def modal(assigns) do
    ...
    >
      <!--Was `"bg-zinc-50/90 fixed inset-0 transition-opacity"`-->
      <div
        id={"#{@id}-bg"}
      ...
      >
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              id={"#{@id}-container"}
              phx-window-keydown={js_exec_data_cancel(@id)}
              phx-key="escape"
              phx-click-away={js_exec_data_cancel(@id)}
              class={[
            ...
            >
              <div class="absolute top-6 right-5">
                <button
                  phx-click={js_exec_data_cancel(@id)}
                  type="button"
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class={class([@margin_classes, @bg_class])}>
  ...
  def input(assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          class([@margin_class, "block w-full rounded-lg text-zinc-900"]),
          "focus:ring-0 sm:text-sm sm:leading-6 phx-no-feedback:border-zinc-300",
  ...
  # TODO: animate flash hiding when closing modal
  ...
  @spec js_exec_data_cancel(String.t()) :: %JS{}
  defp js_exec_data_cancel(id) do
