defmodule CloudDbUiWeb.CoreComponents do
  ...
  defp table_tbody_td_attrs() do
    class =
      Kernel.<>(
        "px-2 py-4 first:rounded-l-lg last:rounded-r-lg text-ellipsis ",
        "overflow-hidden whitespace-nowrap max-w-[8rem]"
      )
  ...
  defp pagination_result_counter(%Flop.Meta{} = meta, count_on_page)
    ...
    "#{meta.total_count} result#{if needs_plural_form?(meta.total_count), do: "s"}."
  end

  # `meta.total_count` is not equal to `count_on_page`
