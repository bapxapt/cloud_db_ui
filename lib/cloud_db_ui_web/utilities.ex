defmodule CloudDbUiWeb.Utilities do
  ...
  after the point. If a passed `value` is not a `%Decimal{}`
  and is not a valid string, return it with no changes.
  """
  @spec format(%Decimal{} | String.t()) :: String.t()
  def format(value), do: format(value, 2)

  def format(value, digits) when is_binary(value) do
    if valid_number_format?(String.trim(value), digits) do
      value
      |> Decimal.new()
      |> format(digits)
    else
  ...
  # A passed `value` is not a `%Decimal{}` and is not a string.
  def format(value, _digits), do: value
