defmodule CloudDbUiWeb.HTML do
  ...
  # Expects a trimmed `input`.
  @spec append_character_counter(String.t(), String.t(), non_neg_integer()) ::
          String.t()
  defp append_character_counter(label, input, limit) do
    ...
    |> Kernel.<>("#{String.length(input)}/#{limit} character")
    |> Kernel.<>("#{if Utilities.needs_plural_form?(limit), do: "s"})")
  end
end
